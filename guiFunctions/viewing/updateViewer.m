function updateViewer( handles )
% This function updates the plots in the GUI. The intput of this function
% is the handle of the  GUI figure.
 
%if getV('DEBUG_MODE'); disp('Function : updateViewer'); end
% -------------------------------------------------------------------------
%If this function is called by accident when no patient data are loade
% to the viewer, it will return without doing nothing
    ...
    orientationMode = getV('orientationMode');
    ...
    Level= getV('Level');
        ...
        switch orientationMode
            case 'ASC' ; % axial, sagital, coronal
                %axes(handles.TopAxes);
                set(gcf,'CurrentAxes',handles.TopAxes);
                plotView('Axial');
                axis off
               
                %axes(handles.BottomLAxes); BottomLAxes -> up left
                ...
                plotView('Coronal');
                axis off
            case 'SAC';  % sagital, axial, coronal
                %axes(handles.TopAxes);
                set(gcf,'CurrentAxes',handles.TopAxes);
                plotView('Sagital');
                axis off
                ...
                plotView('Coronal');
                axis off
            case 'CAS';  % coronal, axial, sagital
                %axes(handles.TopAxes);
                set(gcf,'CurrentAxes',handles.TopAxes);
                plotView('Coronal');
                axis off
        ...
        % -------------------------------------------------------------------------
            switch view
              ...
              case 'Sagital'
                    ...
                    imagesc(imrotate(squeeze(currentPatient.CTimages(:,crossPosition.y,:)),90));
                    pbaspect([1 1 1]),colormap gray,caxis([Level-(Window/2)  Level+(Window/2)]);
