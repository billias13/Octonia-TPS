...
function TPS_v3_OpeningFcn(hObject, ~, handles, varargin)
    ...
    initIsodosePlotInfo(hObject,handles);
    handles = guidata(hObject);
    updateIsodosePlotInfo(hObject,handles);
...
function showVOIS_Callback(hObject, eventdata, handles)
   if strcmp(get(handles.showVOIS, 'Checked'),'on')
        setV('showVOIS',0);
        set(handles.showVOIS, 'Checked','off');
    else 
        setV('showVOIS',1);
        set(handles.showVOIS, 'Checked','on');
    end
    guidata(hObject,handles);
    updateViewer(handles);
...
function showIsodoses_Callback(hObject, eventdata, handles)
    if strcmp(get(handles.showIsodoses, 'Checked'),'on')
        setV('showIsodoses',0);
        set(handles.showIsodoses, 'Checked','off');        
    else 
        setV('showIsodoses',1);
        set(handles.showIsodoses, 'Checked','on');    
    end
    updateIsodosePlotInfo(hObject,handles);
    updateViewer(handles);
...
function setViewMenuShowPlanState(handles, state)
   try
      ...
      set(handles.btnEditIsodoses,'Enable',state);
      set(handles.btnShowDwellProperties,'Enable',state);
    catch e
        disp(e.message);
   end   
...
%This function initializes isodoses legent list and adds it to the handles.
%initially, legents are invisible, but after patient data are loades,this
%legents appear or hide depending on each isodose visible property,value
%and color.
function initIsodosePlotInfo(hObject, handles)
    isodoseProperties   =    getV('isodoseProperties');
    
    %Initialize Isodose labels %
    visibleIsodoses = 0;
    for i=10:-1:1
        ...
        handles.isodoseLabel(i) = uicontrol('style','text','FontSize',11.0,'FontWeight','bold',... 
                  'String', isodoseProperties.Value(i),...
                  'BackgroundColor','Black',...
                  'ForegroundColor',color,...
                  'Visible','on',...
                  'Position', [660 705 - (visibleIsodoses*20) 45 20]);
    end
    guidata(hObject,handles);   
    
    
%A direct copy of the same function exists in the frmEditIsodoses. This 
%functionupdates the legent containing isodoses, depending on color, value
%and visibility.
